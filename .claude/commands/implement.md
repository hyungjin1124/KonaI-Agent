...
6. **테스트 시나리오 설계**
   - Acceptance Criteria 각 항목에 대해 최소 1개 이상의 **구체적 테스트 시나리오**를 설계한다.
   - 시나리오에는 입력 조건, 기대 동작, 테스트 방법(RTL query, userEvent, mock 등)을 명시한다.
   - 우선순위를 부여한다:
     - `must`: AC 직결 — 반드시 테스트 작성
     - `should`: 엣지케이스 — 가능하면 작성
     - `could`: 선택적 — 시간 여유 시 작성
...
|---|---------------------|---------|-----------|---------|
| 1 | ... | {입력 조건} → {기대 동작} | {RTL/userEvent/mock 등} | must |
| 2 | ... | {입력 조건} → {기대 동작} | {RTL/userEvent/mock 등} | should |
```
...
**Stage 3에서 설계한 테스트 시나리오를 기반으로** 테스트를 작성한다.

**테스트 파일 위치**: 컴포넌트와 같은 디렉토리에 `{ComponentName}.test.tsx`

**시나리오 기반 테스트 작성 규칙**:
- `must` 시나리오: 반드시 테스트 작성. 누락 시 Dev Test FAIL.
- `should` 시나리오: 가능하면 테스트 작성.
- `could` 시나리오: 시간 여유 시 작성.
- 테스트를 `describe` 블록으로 시나리오 단위 그룹화한다.
- 기본 smoke test(렌더링 확인)는 시나리오와 별도로 항상 포함한다.
- 커스텀 Hook이 있으면 Hook 단독 테스트도 포함한다.
...
| # | 시나리오 | 우선순위 | 테스트 위치 | 결과 |
|---|---------|---------|-----------|------|
| 1 | {시나리오 설명} | must | {파일명}:L{라인} | PASS |
| 2 | {시나리오 설명} | should | {파일명}:L{라인} | PASS |
