...
export const HandoffIndicator: React.FC<HandoffIndicatorProps> = ({
  ...
  return (
    ...
    >
      <span className="text-sm" role="img" aria-label={fromAgent.name}>{fromAgent.icon}</span>
      <ArrowRight className="w-3.5 h-3.5 text-amber-600 flex-shrink-0" aria-hidden="true" />
      <span className="text-sm" role="img" aria-label={toAgent.name}>{toAgent.icon}</span>
