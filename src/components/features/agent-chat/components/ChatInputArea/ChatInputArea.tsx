...
const SALES_ANALYSIS_CHIPS: SuggestionChip[] = [
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (artifactDataStr) {
      try {
        ...
        return;
      } catch { /* fallthrough to native file */ }
    }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === 'Enter' && !e.shiftKey) {
      ...
      if (inputValue.trim() || attachedFile) {
        onSend(inputValue, attachedFile || undefined);
        setAttachedFile(null);
      }
    }
  };
  ...
  return (
    <div className={`p-4 pb-6 bg-white flex flex-col items-center ${
      ...
      >
        ...
        {/* 입력창 */}
        <div className="relative bg-[#F3F4F6] rounded-xl border border-transparent focus-within:border-[#FF3C42] focus-within:bg-white focus-within:ring-1 focus-within:ring-[#FF3C42] transition-all p-2 flex flex-col gap-1">
          {/* 첨부 파일 칩 - 입력창 내부 */}
          {attachedFile && (
